...
- (UIImageView *)bgImageView {
    if (!_bgImageView) {
        ...
        _bgImageView.center = CGPointMake([UIScreen mainScreen].bounds.size.width/2, _bgImageView.center.y);
...
- (UIButton *)skipButton {
    if (!_skipButton) {
        ...
        _skipButton.titleLabel.font=[UIFont fontWithName:@"Roboto-Light" size:18.0];
...
- (UIView *)viewForPage:(EAIntroPage *)page atXIndex:(CGFloat)xIndex {
    ...
    if(page.titleIconView) {
        ...
        //        rect1.origin.y = 30; //page.titleIconPositionY;
        
        if (IPHONE4S) {
            rect1.origin.x = (rect1.size.width)/1.9;
            rect1.origin.y = 30; //page.titleIconPositionY;
            rect1.size.height=260.0;
            rect1.size.width=138.0;
        }else if(IPHONE6) {
            rect1.origin.x = (self.scrollView.frame.size.width - rect1.size.width)/2;
            rect1.origin.y = 73; //page.titleIconPositionY;
        }else if(IPHONE6PLUS) {
            rect1.origin.x = (rect1.size.width)/1.5;
            rect1.origin.y = 85; //page.titleIconPositionY;
            rect1.size.height=370.0;
            rect1.size.width=185.0;
        } else {
            rect1.origin.x = (self.scrollView.frame.size.width - rect1.size.width)/2;
            rect1.origin.y = 50; //page.titleIconPositionY;
        }
    ...
    if(page.title.length) {
        ...
        titleLabelFrame.size.width=self.scrollView.frame.size.width - 20;
        titleLabelFrame.size.height=titleHeight;
        
        UILabel *titleLabel;
        if (IPHONE4S || IPHONE5S) {
            titleLabelFrame.origin.y=self.bounds.size.height - page.titlePositionY+5;
        }else if(IPHONE6){
            titleLabelFrame.origin.y=self.bounds.size.height - page.titlePositionY-20;
        }else if(IPHONE6PLUS){
            titleLabelFrame.origin.y=self.bounds.size.height - page.titlePositionY-30;
        }
        ...
        titleLabel.font = [UIFont fontWithName:@"Roboto-Bold" size:20.0];
    ...
    if([page.desc length]) {
        CGRect descLabelFrame = CGRectZero;
        descLabelFrame.origin.x=(self.bounds.size.width - page.descWidth)/2;
        descLabelFrame.size.width =page.descWidth;
        descLabelFrame.size.height =500;
        if(page.descWidth != 0) {
            descLabelFrame.origin.x=(self.bounds.size.width - page.descWidth)/2;
            descLabelFrame.size.width =page.descWidth;
            descLabelFrame.size.height =500;
            if (IPHONE4S || IPHONE5S) {
                descLabelFrame.origin.y = self.bounds.size.height - page.descPositionY+10;
            }else if(IPHONE6){
                descLabelFrame.origin.y = self.bounds.size.height - page.descPositionY-15;
            }else if(IPHONE6PLUS){
                descLabelFrame.origin.y = self.bounds.size.height - page.descPositionY-25;
            }
        } else {
            descLabelFrame.origin.x=0;
            descLabelFrame.size.width=self.scrollView.frame.size.width;
            descLabelFrame.size.height=500;
            ...
            if (IPHONE4S || IPHONE5S) {
                ...
                descLabelFrame.origin.y=self.bounds.size.height - page.descPositionY-20;
            }
        }
        
        UITextView *descLabel = [[UITextView alloc] initWithFrame:descLabelFrame];
        if (IPHONE4S || IPHONE5S) {
            descLabel.font = [UIFont fontWithName:@"Roboto-Light" size:13.0];
        }else if(IPHONE6){
            descLabel.font = [UIFont fontWithName:@"Roboto-Light" size:15.0];
        }else if(IPHONE6PLUS){
            descLabel.font = [UIFont fontWithName:@"Roboto-Light" size:16.0];
        }
